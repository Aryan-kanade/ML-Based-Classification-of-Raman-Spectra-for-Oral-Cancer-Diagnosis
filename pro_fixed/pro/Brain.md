...
Last updated: 2026-09-07 (**EXTERNAL AUDIT §34 — CRITICAL: the paired
reference included the spectrum being classified; every pre-2026-09-07
paired number is VOID (0.744 → 0.52) and every paired bundle is
unusable. Also fixed: patient-clustered AUC CIs, CI/point-estimate
decision-rule mismatch, nested model selection for the winner's curse,
selection-biased ensemble flags, reproduce_report XGB fallback, test
SKIP vs FAIL. 72/72 tests, 0 failures**;
previously: FILE > CLEAR TRAINING menu action — §33;
...
**Paired mode**: per patient with both classes, every kept spectrum
becomes `spectrum − own_normal_reference` (optionally PQN against the
reference first). The reference is **LEAVE-ONE-OUT since 2026-09-07**
(§34): a Tumor row uses the mean of ALL the patient's preprocessed
normals, a Normal row uses the mean of that patient's OTHER normals. A
Normal spectrum that is its patient's only normal has no honest
reference and is DROPPED (`n_normals_dropped`); its patient still
contributes Tumor rows. Unpaired patients excluded entirely
(`n_unpaired_excluded`). If the result has <2 classes, `paired_features`
raises with an actionable message (use Standard mode).
**Do NOT "simplify" this back to one shared reference per patient** —
that is the artefact described in §34 and it makes the mode look ~0.22
macro-F1 better than it is while breaking deployment.
...
## 13. Real study results — **ALL PRE-2026-09-07 NUMBERS ARE VOID**

> **The paired-mode results below were an ARTEFACT.** See §34. The
> paired reference included the spectrum being classified; correcting it
> collapsed paired macro-F1 from 0.744 to ~0.52 on the same data. Do NOT
> cite any pre-2026-09-07 paired number anywhere. Standard-mode numbers
> are unaffected by that bug but still predate the trimmed loader
> (gotcha #18) and the site-token hygiene change — re-run before citing.

Hygiene per run: 317 spectra / 72 patients → 18 spike-flagged excluded
(despike off). All study_run artifact folders deleted (§32); the numbers
here are the only record — re-run `reproduce_study.py` to regenerate.

**VOID (kept only so the artefact is recognisable if it resurfaces):**

| Mode | Winner | macro-F1 | AUC | Status |
|---|---|---|---|---|
| Standard | Peak bands + RF | 0.594 | 0.610 | stale pipeline, re-run |
| ~~Paired~~ | ~~Extra Trees~~ | ~~0.702~~ | ~~0.788~~ | **VOID — self-reference artefact** |

**Post-fix verification runs (2026-09-07, `--mini` = PCA+LDA / PCA+LR /
Ensemble only — NOT the full registry, so these are floor values, not
the study result):**

| Mode | Winner | macro-F1 | AUC (patient-clustered CI) | LOPO |
|---|---|---|---|---|
| Standard | PCA + LDA | 0.530 | 0.537 (0.48–0.59) | F1 0.496 |
| Paired | PCA + Logistic | 0.519 | 0.568 (0.52–0.62) | F1 0.537 |

Standard-mode nested model selection (the unbiased estimate of the whole
procedure): **macro-F1 0.504 ± 0.029**. Paired mode no longer beats
standard — its entire former advantage was the artefact.

**A full-registry re-run of both modes is the outstanding task.** Until
then the project has no citable headline number. Literature
(spectrum-level splits, looser): ~0.89/0.85. No external validation.
...
|---|---|---|
| `test_all.py` (88) | loader hygiene (incl. site-token), preprocessing, grouped+repeated CV/ensembles, bundle roundtrip, optimize, ViT forward/checkpoint, clinical stats, biochem, FDR/Friedman + regressions (clinical reproduce path, paired single-preprocess, OOF repeat pooling, patient bootstrap, data-root discovery, device-portability scan; 2026-09-06: CONTIGUOUS calibration-bins regression, auc_power formula regression; 2026-09-07: clear-training menu action) | ~2 min |
...
## 34. 2026-09-07 — EXTERNAL AUDIT: paired self-reference artefact + 5 fixes

An outside review of the scientific core found one critical bug and four
methodology defects. All fixed; six regression tests pin them.
`test_all.py` is now **72/72 passing, 0 failures** (22 skipped for absent
optional deps — skips are reported separately since the old runner
counted them as FAIL and buried the two real failures).

### 34.1 CRITICAL — the paired reference contained the spectrum itself

`paired.paired_features` built `ref = mean(ALL the patient's normals)`
and subtracted it from **every** row, including the Normal rows that
went into it. So a Normal row was a deviation from a mean containing
itself (shrunk toward zero) while a Tumor row was a deviation from a
reference that excluded it (full magnitude). The class signal became
row MAGNITUDE, not biochemistry.

Measured on the real cohort (317 spectra / 72 patients):

| | zero rows | AUC of ‖row‖ ALONE | Extra Trees macro-F1 / AUC |
|---|---|---|---|
| Before | **19** (all Normal) | **0.835** | 0.744 / 0.818 |
| After (leave-one-out) | 0 | 0.515 | 0.523 / 0.553 |

- 19 Normal rows were the **exact zero vector** — patients with a single
  normal, where `row − mean([row]) = 0`. Trivially separable.
- A single scalar (the L2 norm) scored AUC 0.835, HIGHER than the
  reported model AUC of 0.788. The models were reading the artefact.
- Median ‖row‖: Normal 0.186 vs Tumor 0.330 — an arithmetic gap, not a
  biological one.

**Deployment consequence (worse than the inflated CV number):**
`reference_vector()` builds the predict-time reference from OTHER files,
so a real Normal sample is never inside its own reference and arrives at
full magnitude. Train-on-current / test-on-deploy-style simulation:

```
predicted Tumor rate:  Normal 0.80   Tumor 0.79
```

The model called 4 of 5 healthy sites cancerous. **Every paired .joblib
saved before 2026-09-07 is unusable — delete and retrain.**

Fix: leave-one-out reference (§4). Guard added for degenerate cohorts.
Pinned by `test_paired_reference_excludes_self` (asserts no zero rows
AND that ‖row‖ can no longer separate the classes, |AUC−0.5| < 0.20).

### 34.2 AUC CIs ignored patient clustering

`bootstrap_ci` resampled patients; `delong_auc_ci` treated 2–5 spectra
per subject as independent — two headline numbers on contradictory
assumptions, the AUC interval far too narrow (pseudo-replication).
`clinical.delong_auc_ci(y, p, groups=...)` now runs a cluster bootstrap
over patients (point estimate unchanged, `se` = bootstrap SD); falls
back to analytical DeLong with <3 clusters. Wired into reproduce_study
(pooled + locked exam) and both GUI sites.
Pinned by `test_delong_ci_is_patient_clustered`.

### 34.3 The CI described a different classifier than the point estimate

Headline macro-F1 came from fold metrics at the per-fold **tuned
threshold**; its bootstrap CI came from `np.argmax(oof)` — a hard 0.5
cut. New `modeling.oof_predictions(result, valid)` applies the tuned
threshold when one exists, else argmax; replaces the raw argmax at all
3 sites (reproduce_study, GUI CI, GUI McNemar).
Pinned by `test_oof_predictions_match_tuned_threshold`.

### 34.4 Winner's curse

`evaluate_models` reports the MAX over ~24 models scored on one set of
folds, then reuses the winner's own folds as its performance and CI —
optimistic by an unknown amount. Added
`modeling.nested_model_selection()`: the entire model choice re-runs
inside every outer fold, scored on untouched patients. Exposed as
`reproduce_study.py --nested-selection`; summary.txt now states the
optimism explicitly. First real measurement (standard, --mini):
pooled 0.530 vs **nested 0.504 ± 0.029**.
Pinned by `test_nested_model_selection_is_not_optimistic`.

### 34.5 Ensemble members picked on the folds they are scored on

The top-3 voting/stacked rows choose members via CV scores from the same
folds. Cannot be fixed without nesting the whole registry per fold, so
`ModelResult.selection_biased` flags both rows and reports mark them
`* members picked on these same folds`.
Pinned by `test_ensemble_rows_flagged_selection_biased`.

### 34.6 `reproduce_report.py` XGBoost fallback never fired

`make(dim, None)` returns a **Pipeline whose clf STEP is None**, never
None itself, so `if v is None` never matched, XGB rows survived, and
`cross_val_predict` raised instead of degrading. Now inspects
`named_steps["clf"]`.
Pinned by `test_report_models_degrade_without_xgboost`.

### 34.7 Test-suite hygiene

- `qt_compat` raised `No module named 'PySide6'`, implying PySide6
  specifically was required; now raises a clear error naming all three
  bindings and pointing headless users at the CLI entry points.
- Runner reports missing optional deps as **SKIP**, not FAIL.
- Two assertions were dependency-blind (saliva preset needs pybaselines;
  Friedman needs ≥3 models, i.e. xgboost for the mini subset).
- `_make_clinical_tree(root, paired_ready=True)` gives Patient_2/3 a
  second normal — under leave-one-out the base tree leaves only one
  patient with Normal rows, so grouped CV folds went single-class.

### 34.8 Outstanding

1. **Full-registry re-run of both modes** — §13's post-fix numbers are
   `--mini` floors. No citable headline exists until this is done.
2. Delete/retrain any paired bundle saved before 2026-09-07.
3. The write-up needs rewriting, not re-running: paired mode was the
   central claim and it no longer beats standard. Standard mode is now
   the primary result, and it is weak.

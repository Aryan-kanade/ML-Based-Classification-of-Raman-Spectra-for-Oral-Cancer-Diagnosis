...
def main(argv=None) -> int:
    ...
    ap.add_argument("--nested-selection", action="store_true",
                    help="also run the nested model-selection check "
                         "(re-picks the winner inside every outer fold; "
                         "slow, but removes the winner's-curse optimism)")
    ...
    if args.mode in ("paired", "paired-pqn"):
        ...
        print(f"[reproduce] {args.mode}: {pd_.X.shape[0]} deviation "
              ...
              f"({pd_.n_unpaired_excluded} patients without a normal "
              f"reference, {pd_.n_normals_dropped} single-normal "
              f"spectra dropped — no leave-one-out reference)")
        paired_counts = {
            "n_rows": int(pd_.X.shape[0]),
            "n_patients": int(pd_.n_patients),
            "n_unpaired_excluded": int(pd_.n_unpaired_excluded),
            "n_normals_dropped": int(pd_.n_normals_dropped)}
    else:
    ...
    for r in sorted((x for x in results if x.error is None),
        ...
        lines.append(f"  {r.name:<34} {r.macro_f1():.3f}"
                     + ("  * members picked on these same folds"
                        if getattr(r, "selection_biased", False) else ""))
    lines += [f"\nWinner: {winner.name}",
              f"  sensitivity {winner.macro.get('sens', (0,))[0]:.3f}"
              f" · specificity "
              f"{winner.macro.get('spec', (0,))[0]:.3f}"
              f" · macro-F1 {winner.macro_f1():.3f}",
              "  NOTE: this winner was picked as the maximum over "
              f"{sum(1 for x in results if x.error is None)} models "
              "scored on these same folds, so it carries selection "
              "optimism (winner's curse); the nested-selection number "
              "below is the unbiased one."]
    if args.nested_selection:
        try:
            ns = modeling.nested_model_selection(
                X, y, model_names=model_names, k_folds=args.folds,
                seed=args.seed, groups=g, wavenumbers=wn_cropped)
            if ns["fold_f1s"]:
                lines.append(
                    f"  NESTED model selection: macro-F1 "
                    f"{ns['mean_f1']:.3f} ± {ns['std_f1']:.3f} "
                    f"(most-picked: {ns['chosen']}) — the honest "
                    "estimate of this whole procedure")
                nested_out = ns
                print(f"[reproduce] nested selection: "
                      f"{ns['mean_f1']:.3f} ({ns['chosen']})")
        except Exception as exc:
            lines.append(f"  nested selection skipped: {exc}")
    ...
    if have_oof:
        ...
        gv = (np.asarray(winner.groups)[valid]
              if getattr(winner, "groups", None) else None)
        auc, _se, lo, hi = clin.delong_auc_ci(yv, pv, groups=gv)
        _ci_kind = ("patient-clustered bootstrap" if gv is not None
                    else "DeLong")
        lines.append(f"  AUC {auc:.3f} ({_ci_kind} 95% CI "
                     f"{lo:.2f}-{hi:.2f})")
        pred_oof = modeling.oof_predictions(winner, valid)
    ...
    if (g is not None and len(set(g)) >= 7 and winner.pipeline is not None
        ...
        try:
            ...
            if len(winner.classes) == 2:
                ...
                a_, _se_, lo_, hi_ = clin.delong_auc_ci(
                    yv_, proba_, groups=[g[i] for i in rows_te])
                lines.append(
                    f"  sens {tp / max(tp + fn_, 1):.3f} · "
                    f"spec {tn / max(tn + fp_, 1):.3f} · "
                    f"AUC {a_:.3f} (patient-clustered 95% CI "
                    f"{lo_:.2f}-{hi_:.2f})")

...
def prepare_dataset(root: str, mode: str = "standard"):
    ...
    meta = {"n_spectra": len(y), "n_patients": len(set(g)) if g else 0,
            "mode": mode}
    if mode in ("paired", "paired-pqn"):
        # 2026-09-07: paired mode now drops single-normal spectra (no
        # honest leave-one-out reference) — record it so a run's n is
        # explainable from its manifest alone.
        meta["n_unpaired_excluded"] = int(pd_.n_unpaired_excluded)
        meta["n_normals_dropped"] = int(pd_.n_normals_dropped)

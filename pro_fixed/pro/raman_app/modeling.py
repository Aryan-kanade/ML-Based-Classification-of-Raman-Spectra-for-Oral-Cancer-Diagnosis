...
class ModelResult:
    ...
    error: str | None = None
    # True when this row's MEMBERS were chosen using the same folds it is
    # scored on (the top-3 ensembles) — its score is not comparable with
    # the individual rows.  Reports flag it (2026-09-07).
...
def evaluate_models(X: np.ndarray, y: list[str],
    ...
    if ens_spec is not None and len(pipelines) >= 2:
        ...
        # NOTE (2026-09-07): the three members are chosen using scores
        # from the SAME folds the ensembles are then scored on, so both
        # ensemble rows carry a selection advantage the individual rows
        # do not.  Members cannot be re-picked per fold without nesting
        # the whole registry inside every fold, so instead the bias is
        # recorded on the result and surfaced in the reports; use
        # `nested_model_selection` for the unbiased number.
...
def nested_model_selection(X: np.ndarray, y: list[str],
                           model_names: list[str] | None = None,
                           k_folds: int = 5, seed: int = RANDOM_STATE,
                           groups: list[str] | None = None,
                           wavenumbers: np.ndarray | None = None,
    ...
    """
    Honest estimate of "the score you get by running evaluate_models and
    taking its winner" — the model CHOICE is made inside each outer fold.

    `evaluate_models` scores up to 24 models on one set of folds and
    reports the maximum.  The maximum of many noisy estimates is biased
    upward (winner's curse / multiple comparisons), and the winner's own
    fold scores are then reused as its performance and its CI — so the
    headline is optimistic by an unknown amount.  Here every outer fold
    runs the WHOLE selection on its training patients only (an inner
    grouped CV over every candidate model, including hyperparameter and
    threshold tuning) and scores the model that inner CV picked on the
    untouched outer patients.

    Returns {mean_f1, std_f1, fold_f1s, fold_choices, chosen} where
    `fold_choices` records the model selected per fold and `chosen` is
    the most frequently selected name.  Compare it to the pooled
    `evaluate_models` number: the gap IS the selection optimism.
    """
    from sklearn.metrics import f1_score

    classes = sorted(set(y))
    ye = _encode(y, classes)
    k = int(np.clip(k_folds, 2, min(10, min(Counter(y).values()))))
    if groups is not None:
        g = np.asarray(groups)
        k = int(np.clip(k, 2, len(set(groups))))
        outer = StratifiedGroupKFold(n_splits=k, shuffle=True,
                                     random_state=seed)
        splits = list(outer.split(X, ye, g))
    else:
        g = None
        splits = list(StratifiedKFold(n_splits=k, shuffle=True,
                                      random_state=seed).split(X, ye))

    fold_f1s: list[float] = []
    fold_choices: list[dict] = []
    for fold_i, (tr, te) in enumerate(splits, start=1):
        if progress_cb:
            progress_cb(int(100 * (fold_i - 1) / len(splits)),
                        f"nested selection: fold {fold_i}/{len(splits)}")
        ytr = [y[i] for i in tr]
        gtr = [groups[i] for i in tr] if groups is not None else None
        # the ENTIRE selection, on training patients only
        try:
            _res, win = evaluate_models(
                X[tr], ytr, model_names=model_names,
                k_folds=k_folds, seed=seed, groups=gtr,
                repeats=1, wavenumbers=wavenumbers)
        except Exception:
            continue
        if win.pipeline is None:
            continue
        proba = win.pipeline.predict_proba(X[te])
        if len(classes) == 2 and win.threshold is not None:
            pred = np.where(proba[:, 1] >= float(win.threshold), 1, 0)
        else:
            pred = np.argmax(proba, axis=1)
        f1 = float(f1_score(ye[te], pred, average="macro"))
        fold_f1s.append(f1)
        fold_choices.append({"fold": fold_i, "model": win.name,
                             "inner_f1": float(win.macro_f1()),
                             "outer_f1": f1})
    if not fold_f1s:
        return {"mean_f1": float("nan"), "std_f1": float("nan"),
                "fold_f1s": [], "fold_choices": [], "chosen": None}
    picked = Counter(c["model"] for c in fold_choices).most_common(1)[0][0]
...
def oof_predictions(result, valid=None) -> np.ndarray:
    """
    Out-of-fold hard predictions using the SAME decision rule the fold
    metrics used: the tuned binary threshold when the result carries one,
    otherwise argmax.

    (2026-09-07: the reported macro-F1 came from fold metrics scored at
    the per-fold tuned threshold, while its bootstrap CI was computed
    from a plain `argmax` of the pooled OOF — a hard 0.5 cut.  The
    interval therefore described a DIFFERENT classifier than the point
    estimate and could sit off-centre from it.  Everything now goes
    through this helper.)
    """
    proba = np.asarray(result.oof_proba)
    if valid is not None:
        proba = proba[valid]
    thr = getattr(result, "threshold", None)
    if len(getattr(result, "classes", [])) == 2 and thr is not None:
        return np.where(proba[:, 1] >= float(thr), 1, 0)

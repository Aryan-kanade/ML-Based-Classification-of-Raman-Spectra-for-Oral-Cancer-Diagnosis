...
def delong_auc_ci(y_true, p_pos: np.ndarray, z: float = 1.96,
                  groups=None, n_boot: int = 2000,
                  seed: int = 42):
    """
    ROC-AUC with a 95% confidence interval.  Returns (auc, se, lo, hi);
    NaNs when a class is empty.

    Without `groups` the interval is the classical DeLong one
    (structural components, tie-corrected) — valid only for INDEPENDENT
    observations.

    With `groups` (one id per row, e.g. patient) the interval comes from
    a CLUSTER bootstrap that resamples whole patients, matching
    `modeling.bootstrap_ci`.  Raman cohorts carry 2-5 spectra per
    subject, so the independence assumption behind the analytical SE is
    false and it reports an interval that is far too narrow
    (pseudo-replication).  The point estimate is unchanged; `se` becomes
    the bootstrap standard deviation.  (2026-09-07: the F1 interval was
    already patient-clustered while the AUC interval was not — the two
    headline numbers used different, inconsistent assumptions.)
    """
    y = np.asarray(y_true)
    p = np.asarray(p_pos, dtype=float)
    ok = ~np.isnan(p)
    ...
    if g is None:
        se = float(np.sqrt(np.var(v10, ddof=1) / m
                           + np.var(v01, ddof=1) / n))
        return (auc, se, auc - z * se, min(1.0, auc + z * se))

    rows_by = [np.flatnonzero(g == u) for u in np.unique(g)]
    n_blocks = len(rows_by)
    if n_blocks < 3:                       # too few clusters to resample
        se = float(np.sqrt(np.var(v10, ddof=1) / m
                           + np.var(v01, ddof=1) / n))
        return (auc, se, auc - z * se, min(1.0, auc + z * se))
    ...
    for _ in range(int(n_boot)):
        ...
        boots.append(_auc_fast(yb, p[idx]))
    if len(boots) < 20:
        se = float(np.sqrt(np.var(v10, ddof=1) / m
                           + np.var(v01, ddof=1) / n))
        return (auc, se, auc - z * se, min(1.0, auc + z * se))
    boots = np.asarray(boots)
    return (auc, float(np.std(boots, ddof=1)),
            float(np.percentile(boots, 2.5)),
            float(min(1.0, np.percentile(boots, 97.5))))


def _auc_fast(y: np.ndarray, p: np.ndarray) -> float:
    """Tie-corrected Mann-Whitney AUC (bootstrap inner loop)."""
    r = _midrank(p)
    n_pos = int((y == 1).sum())
    n_neg = len(y) - n_pos

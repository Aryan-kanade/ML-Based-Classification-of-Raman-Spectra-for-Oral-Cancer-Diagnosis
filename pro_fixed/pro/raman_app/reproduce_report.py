...
def report_models() -> "OrderedDict[str, Pipeline]":
    ...
    if not modeling.HAS_XGB:                    # graceful degradation
        # 2026-09-07: `make(dim, None)` returns a Pipeline whose LAST STEP
        # is None — never None itself — so the old `if v is None` filter
        # never matched, the XGB rows survived, and cross_val_predict
        # raised ("estimator must implement fit and predict") instead of
        # degrading.  Inspect the classifier step, not the pipeline.
        for k in [k for k, v in out.items()
                  if v.named_steps.get("clf") is None]:
            del out[k]
    return out
